...
const AUDIO_DETAIL = gql`
  query AudioDetail($id: ID!) {
    product(id: $id, idType: DATABASE_ID) {
      ...
      # ACF meta like other cleaned components
      ... on SimpleProduct {
        price
        regularPrice
        salePrice
...
export default function AudioDetail() {
  ...
  const { data, loading, error } = useQuery(AUDIO_DETAIL, {
  ...
  if (error || !view)
    return (
      <div className="p-6 text-center">
        <p className="text-red-600 mb-4">Error: {error?.message || "Failed to load audio device."}</p>
