...
const STORAGE_DETAIL = gql`
  query StorageDetail($id: ID!) {
    product(id: $id, idType: DATABASE_ID) {
      ...
      # ACF meta: align with other cleaned detail pages
      ... on SimpleProduct {
        price
        regularPrice
        salePrice
...
const toView = (p) => {
  ...
  const brand_display = p?.meta?.brand?.trim?.() || "Others";
  const category_display = p?.meta?.category?.trim?.() || "Others";
  const specs_text = p?.meta?.specs?.trim?.() || "";

  const cur = num(p.salePrice || p.price || p.regularPrice);
  const crossed = p.onSale && p.regularPrice ? num(p.regularPrice) : null;
...
export default function StorageDetail() {
  ...
  const { data, loading, error } = useQuery(STORAGE_DETAIL, {
    variables: { id: String(id) },
    fetchPolicy: "cache-and-network",
  });

  const view = useMemo(() => toView(data?.product), [data]);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      setAdding(true);
      const res = await mutateAddToCart({ variables: { productId: view.product_id, quantity: 1 } });
  ...
  if (error || !view)
    return (
      <div className="p-6 text-center">
        <p className="text-red-600 mb-4">Error: {error?.message || "Failed to load storage device."}</p>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          {/* brand & category under the title (same as others) */}
          ...
          {/* Only Slug + Specs (cleaned detail like others) */}
