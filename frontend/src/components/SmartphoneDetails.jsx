...
export default function SmartphoneDetail() {
  const { id } = useParams(); // /smartphone/:id -> Woo databaseId
  ...
  const view = useMemo(() => {
    ...
    return {
      ...
      specs_text: specs, // shown next to Slug
      slug: p.slug,
  ...
  if (error || !view)
    return (
      <div className="p-6 text-center">
        ...
        <button className="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300" onClick={() => { scrollToTop(); navigate(-1); }}>
          Go Back
        </button>
      </div>
    );
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          {/* brand & category */}
          <p className="text-gray-600">{brand_display}</p>
          {category_display ? <p className="text-gray-600 mb-2">{category_display}</p> : <div className="mb-2" />}
          ...
          {/* SPECS next to SLUG (from ACF/Woo meta) */}
