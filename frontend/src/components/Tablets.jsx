...
// Adjust to your Woo/WordPress category slug for tablets
const TABLETS_CATEGORY_SLUG = "tablets";

/* ----------------------- GraphQL ----------------------- */

const LIST_TABLET_PRODUCTS = gql`
  query TabletProducts(
    ...
    products(
      ...
      nodes {
        ...
        # Pull the same ACF meta structure you used on phones/laptops
        ... on SimpleProduct {
          price
          regularPrice
          salePrice
...
const money = (raw) => {
  ...
  return n == null ? "" : `KSh ${n.toLocaleString()}`;
};

const normalizeNode = (n) => {
  // ACF meta is the source of truth
  const brandMeta = n?.meta?.brand?.trim?.() || "";
  const categoryMeta = n?.meta?.category?.trim?.() || "";
  const specs = n?.meta?.specs?.trim?.() || "";

  // Keep your special bucket
  const brand_display =
    /tablets\s*for\s*kids/i.test(categoryMeta) || /tablets\s*for\s*kids/i.test(brandMeta)
      ? "Tablets for Kids"
      : brandMeta || "Others";
  ...
  return {
    ...
    brand_display,
    brand: brand_display,
    category_display: categoryMeta,
    specs_text: specs, // not shown on list, only on detail page
    price_display: money(current),
...
export default function Tablets() {
  ...
  const [count, setCount] = useState(null); // unknown without count plugin
  ...
  const { data, loading, error, refetch, fetchMore } = useQuery(LIST_TABLET_PRODUCTS, {
    variables: {
      categorySlugs: [TABLETS_CATEGORY_SLUG],
      first: pageSize,
  ...
  // Client brand filter (no specs in list)
  const filtered = useMemo(() => {
    return baseItems.filter((t) => {
      const brandOK =
        filter === "All" ||
        (t.brand_display || "Others").toLowerCase() === filter.toLowerCase();
      const searchOK =
        !search ||
        [t.name, t.brand_display, t.category_display]
          .filter(Boolean)
  ...
  const grouped = useMemo(() => {
    ...
    for (const t of filtered) {
      const key = t.brand_display || "Others";
  ...
  // show filtered items in summary (count remains nullable)
  useEffect(() => {
    setItems(filtered);
    setCount(null);
  }, [filtered]);

  // reset page and cursor when filters/order change
  useEffect(() => {
    setPage(1);
  ...
  const handleBuyNow = async (tablet) => {
    ...
    try {
      const res = await mutateAddToCart({
        variables: { productId: tablet.product_id, quantity: 1 },
      });
  ...
  useEffect(() => {
    if (error) {
      ...
      console.error("GraphQL error (Tablets):", error);
    }
  }, [error]);
        ...
        <section key={brand} className="mb-12">
          ...
          <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {grouped.get(brand)?.map((t) => {
              ...
              const priceText =
                t.price_display || (t.price_min_ksh != null ? `KSh ${t.price_min_ksh.toLocaleString()}` : "—");
                ...
                >
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <p className="text-gray-600 text-sm mt-1">
                      {[t.brand_display, t.category_display].filter(Boolean).join(" • ") || "—"}
                    </p>

                    <div className="mt-3 flex items-center gap-2">
                      <span className="text-blue-600 font-bold">{priceText}</span>
                      {oldPriceText && <span className="text-gray-400 line-through text-xs">{oldPriceText}</span>}
                    </div>

                    <div className="mt-auto grid grid-cols-2 gap-2 pt-4">
                      <button
                        className="rounded-md border bg-white hover:bg-gray-50 text-gray-900 py-2 px-3 text-sm transition"
                        onClick={(e) => { e.stopPropagation(); goToDetails(t.id); }}
                      >
                        View Details
                      </button>
                      <button
