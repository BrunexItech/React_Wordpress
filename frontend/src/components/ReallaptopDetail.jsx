// src/pages/ReallaptopDetail.jsx
...
const LAPTOP_DETAIL = gql`
  query LaptopDetail($id: ID!) {
    product(id: $id, idType: DATABASE_ID) {
      ...
      # Pull ACF meta (same pattern as Budget Smartphones)
      ... on SimpleProduct {
        price
        regularPrice
        salePrice
...
const deriveDetail = (p) => {
  ...
  return {
    ...
    specs_text, // shown on detail page
    slug: p.slug,
...
export default function ReallaptopDetail() {
  ...
  useEffect(() => { scrollToTop("auto"); }, [id]);

  const { data, loading, error } = useQuery(LAPTOP_DETAIL, {
  ...
  if (error || !view)
    return (
      <div className="p-6 text-center">
        <p className="text-red-600 mb-4">Error: {error?.message || "Failed to load laptop."}</p>
        <button
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          {/* Brand & category (same style as Budget Smartphones detail) */}
