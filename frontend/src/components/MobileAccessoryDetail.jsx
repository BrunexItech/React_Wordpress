...
const PRODUCT_DETAIL = gql`
  query MobileAccessoryDetail($id: ID!) {
...
const deriveView = (p) => {
  ...
  const brand_display = p?.meta?.brand?.trim?.() || "";
...
export default function MobileAccessoryDetail() {
  ...
  if (error || !view)
    return (
      <div className="p-6 text-center">
        <p className="text-red-600 mb-4">Error: {error?.message || "Failed to load accessory."}</p>
  ...
  const {
    name, brand_display, category_display, image, price_display,
    price_min_ksh, price_max_ksh, specs_text, slug, product_id,
  } = view;
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        ...
        <div>
          ...
          {/* SPECS (from ACF meta) next to SLUG */}
